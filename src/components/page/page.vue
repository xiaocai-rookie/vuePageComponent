<template>
    <div class="pagenation">
        <ul>
            <li @click="pre"> 上一页 </li>
            <li v-for="(item,index) in show_page_numbers" :class="{active: item==current_index}" @click="setIndex(item)">{{item}}</li>
    ...
    export default {
        ...
        methods:{
            refreshPage(currentIndex){
                ...
                currentIndex?currentIndex = currentIndex: currentIndex = this.current_index;
                this.$emit('myindex',currentIndex);//通信将当前的index 通过myindex方法传出去
            ...
            setIndex(index){
                ...
                this.refreshPage(this.current_index);
            }
        }
    }
</script>
